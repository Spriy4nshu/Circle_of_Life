...
# Finding the node that needs to be survayed using the belief list for prey
...
# Prey belief update which incorporates faulty drone setting and how to overcome it
...
# Finding the node that needs to be survayed using the belief list for predator
def predator_belief_target(belief):
    max_prob = max(belief)
    probable_nodes = []
    for x in range(len(belief)):
        if belief[x] == max_prob:
...
# Predator belief update which incorporates faulty drone setting and how to overcome it
...
def agent8b(ndict, clist):
    ...
    prey_belief = list(np.repeat(1/49, 50))
    prey_belief[agent_pos] = 0
    pred_belief = list(np.zeros(50))
    pred_belief[predator_pos] = 1
    list_of_nodes = list(np.arange(50))
    steps = 0
    win_condition = 'out of steps'
    # flag = False
    while steps < 5000:
        probable_prey_pos = prey_belief_target(prey_belief)
        prey_belief = prey_belief_update(probable_prey_pos, clist, prey_belief, prey_pos, "drone_check")
        probable_predator_pos = predator_belief_target(pred_belief)
        pred_belief = predator_belief_update(probable_predator_pos, clist, pred_belief, probable_predator_pos, "drone_check")
        print(sum(pred_belief))
        # agents move
        ndict[agent_pos]['state'] = -1
        ...
        if predator_pos == agent_pos:
            win_condition = 'Loss'
            break
        prey_belief = prey_belief_update(probable_prey_pos, clist, prey_belief, prey_pos, "agent_move")
        pred_belief = predator_belief_update(agent_pos, clist, pred_belief, probable_predator_pos, "agent_move")
        print(sum(pred_belief))
        # predators move
        predator_pos = predator_movement(predator_pos, agent_pos, clist)
        pred_belief = predator_belief_update(agent_pos, clist, pred_belief, probable_predator_pos, "predator_move")
        print(sum(pred_belief))
        ndict[predator_pos]['state'] = -10
        ...
        prey_belief = prey_belief_update(probable_prey_pos, clist, prey_belief, prey_pos, "prey_move")
        ndict[prey_pos]['state'] = 10
        if prey_pos == agent_pos:
            win_condition = 'Win'
            break
...
for x in range(30):
    ...
    for y in range(100):
        steps, win_condition, ndict = agent8b(ndict, clist)
        steps1 += steps
...
print(no_of_steps)
print('done')
data.to_csv('Agent08b.csv', index=False)
