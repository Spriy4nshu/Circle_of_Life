...
# Agent method that runs away from the prey and tries to catch the prey with the given agent movement logic
def agent1(ndict, clist):
...
# Extracting data and generating 30 different graphs and running the simulation 100 times on each graph and finding the varience and standrad deviation
...
for x in range(30):
    ...
    for y in range(100):
        steps, win_condition, ndict = agent1(ndict, clist)
        steps1 += steps
...
print(no_of_steps)
print('done')
data.to_csv('Agent01.csv', index=False)
