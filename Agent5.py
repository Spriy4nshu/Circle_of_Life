...
def agent5(ndict, clist):
    ...
    belief = list(np.zeros(50))
    belief[predator_pos] = 1
    list_of_nodes = list(np.arange(50))
    steps = 0
    win_condition = 'out of steps'
    # flag = False
    while steps < 5000:
        probable_predator_pos = predator_belief_target(belief)
        belief = predator_belief_update(probable_predator_pos, clist, belief, probable_predator_pos, "drone_check")
        flag = True
        print(sum(belief))
        # agents move
        ndict[agent_pos]['state'] = -1
        ...
        belief = predator_belief_update(agent_pos, clist, belief, probable_predator_pos, "agent_move")
        print(sum(belief))
        # predators move
        predator_pos = predator_movement(predator_pos, agent_pos, clist)
        belief = predator_belief_update(agent_pos, clist, belief, probable_predator_pos, "predator_move")
        print(sum(belief))
        ndict[predator_pos]['state'] = -10
...
for x in range(30):
    ...
    for y in range(100):
        steps, win_condition, ndict = agent5(ndict, clist)
        steps1 += steps
...
print(no_of_steps)
print('done')
data.to_csv('Agent05-new.csv', index=False)
